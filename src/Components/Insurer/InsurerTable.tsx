...
import { Paper, Box, Typography, Card, Collapse, IconButton, Toolbar, Tooltip,
...
import { InsurerRowValues, InsurerTableProps } from '../Scaffold/MGAValues';
...
function Row(props: { row: InsurerRowValues }) {
    const { row } = props;
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                <TableCell>
                    <Link to={`/insurers/${row.id}`}>
                    {row.name}
                    </Link>
                </TableCell>
                <TableCell>{row.FEIN}</TableCell>
                <TableCell>{row.status}</TableCell>
                <TableCell>{row.legacyId}</TableCell>
                <TableCell>{row.domicileState}</TableCell>
                ...
                >
                    <Collapse in={open} timeout='auto' unmountOnExit>
                        <Card sx={{ margin: 1, p:1, borderColor:'primary.main' }}>
                            ...
                            <Typography>
                                Max Term: {row.maxPolicyTerm}
                            </Typography>
                            <Typography>
                                Min Term: {row.minPolicyTerm}
                            </Typography>
                            <Typography>   
                                Agent Renewal Days: {row.renewalDaysAgent}
                            </Typography>
                            <Typography>
                                Direct Bill Renewal Days: {row.renewalDaysDirectBill}
                            </Typography>
                        </Card>
                    </Collapse>
                </TableCell>
                ...
                >
                    <Collapse in={open} timeout='auto' unmountOnExit>
                        <Card sx={{ margin: 1, p:1, borderColor:'primary.main' }}>
                            ...
                            <Typography>
                                NAIC Group: {row.NAICGroup}
                            </Typography>
                            <Typography>
                                NAIC Code: {row.NAICCode}
                            </Typography>
                            <Typography>   
                                NAIC Group Name: {row.NAICGroupName}
                            </Typography>
                            <Typography>
                                AM Best ID: {row.AMBestID}
                            </Typography>
                            <Typography>
                                AM Best Rating: {row.AMBestRating}
                            </Typography>
                        </Card>
                    </Collapse>
                </TableCell>
...
//---------------  Insurer Table Function components ---------------------------------------------
export default function InsurerTable( insurerTableProps: InsurerTableProps ) {

    //Set table 'rows' to insurerRows if insurerRows is defined, otherwise set to empty array 
    const { insurerRows } = insurerTableProps;
    const insurerRowsUndefined = (insurerRows === undefined);
    const rows : InsurerRowValues[] = !insurerRowsUndefined ? insurerRows : [];
